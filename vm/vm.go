...
func Create(vmType string, env *vmimpl.Env) (*Pool, error) {
	typ, ok := vmimpl.Types[vmType]
	if !ok {
		return nil, fmt.Errorf("unknown instance type '%v'", vmType)
	}
...
func InitGuestComm(vmType, commType string, index int) (comm.GuestComm, error) {
	if vmType == "qemu" && commType == "virtio" {
		return qemu.InitVirtioGuest(index)
	} else {
		return nil, fmt.Errorf("unsupported guest comm %v/%v", vmType, commType)
	}
}

func (inst *Instance) LoadSnapshot() error {
	return inst.impl.LoadSnapshot()
}

func (inst *Instance) SaveSnapshot() error {
	return inst.impl.SaveSnapshot()
}

func (inst *Instance) CommType() string {
	return inst.impl.CommType()
}

func (inst *Instance) Comm(index int) comm.HostComm {
	return inst.impl.Comm(index)
}

func (inst *Instance) Pause() error {
	return inst.impl.Pause()
}
