...
static void initialize_tun(void)
{
	...
	if (!flag_net_injection)
		return;
#endif
	// Tun device path is per test executor
	char tun_path[64];
	sprintf(tun_path, "/dev/net/tun%lld", procid);
	tunfd = open(tun_path, O_RDWR | O_NONBLOCK);
	if (tunfd == -1) {
#if SYZ_EXECUTOR
...
static void sandbox_common()
{
	...
	rlim.rlim_cur = rlim.rlim_max = (200 << 20) +
					(kMaxThreads * (kCoverSize + kMemtraceSize + kSctraceSize) + kExtraCoverSize) * sizeof(void*);
#else
	rlim.rlim_cur = rlim.rlim_max = (200 << 20);
#endif
...
static int namespace_sandbox_proc(void* arg)
{
	...
	// let attacker and victim stays in the same path
	if (system("pwd"))
		fail("system");
