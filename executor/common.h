...
static void loop(void)
{
	...
	for (;; iter++) {
		...
		receive_execute();
#endif
		if ((IS_ATTACKER(procid) && flag_interleave) ||
		    (IS_VICTIM(procid) && !flag_interleave)) {
				struct timeval timeout;
				timeout = {
					.tv_sec = (long)(program_timeout_ms/1000),
					.tv_usec = (long)(1000*(program_timeout_ms%1000))
				};
				debug("waiting....\n");
				exec_wait(timeout);
				debug("running!\n");
		...
		if (pid < 0)
			fail("clone failed");
		if (pid == 0) {
			const int done_fd = 150;
			close(done_pipe[0]);
			dup2(done_pipe[1], done_fd);
#if SYZ_EXECUTOR || SYZ_USE_TMP_DIR
			if (chdir(cwdbuf))
				fail("failed to chdir");
#endif
			...
			if (IS_ATTACKER(procid)) {
				debug("notify...");
				exec_notify();
			}
			if (write(done_fd, "1", 1) != 1) {
				debug("cannot write to done");
			}
#endif
			// freeze
			while(1) {
...
#endif
		if (flag_interleave || flag_victim_interleave) {
			program_timeout_ms = interleave_program_timeout_ms;
		}
		for (;;) {
			char buf = '\0';
			if (read(done_pipe[0], &buf, 1) == 1)
				break;
			sleep_ms(1);
#if SYZ_EXECUTOR && SYZ_EXECUTOR_USES_SHMEM
...
#endif
			// TODO: do we need to kill blocking/hanging process?
